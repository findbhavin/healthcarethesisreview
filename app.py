...
def email_send_pdf():
    ...
    if not result.get("payment_verified"):
        return jsonify({
            "error": "Full report is available only after payment. Please complete payment first.",
...
def payment_verify():
    ...
    if review_id in _review_store:
        ...
        import threading
        threading.Thread(
            target=_deliver_staged_report,
            args=(review_id, _review_store[review_id].get("invoice", {})),
            daemon=True,
        ).start()
...
def payment_check_order():
    ...
    if captured:
        ...
        _review_store[review_id]["payment_verified"] = True
        _review_store[review_id]["payment_id"] = payment_id
        _review_store[review_id].setdefault("invoice", {
            "invoice_id": invoice_id,
            "order_id": order_id,
            "payment_id": payment_id,
            "amount_paise": PAYMENT_AMOUNT_PAISE,
            "currency": PAYMENT_CURRENCY,
            "description": PAYMENT_DESCRIPTION,
            "paid_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
        })
        logger.info(f"Payment confirmed via check-order for review {review_id}, payment {payment_id}")
        import threading
        threading.Thread(
            target=_deliver_staged_report,
            args=(review_id, _review_store[review_id]["invoice"]),
            daemon=True,
        ).start()
...
def payment_send_invoice():
    ...
    if not result:
        return jsonify({"error": "Review not found."}), 404
    if not result.get("payment_verified"):
        return jsonify({"error": "Invoice can only be sent after successful payment."}), 402
...
# ---------------------------------------------------------------------------
# Dunning management — stage report for 24-hour payment retry window
# ---------------------------------------------------------------------------

@app.route("/payment/stage", methods=["POST"])
def payment_stage():
    """
    Called when payment is dismissed/failed.  Stages the generated PDF and
    user metadata to GCS so the user can retry payment within 24 hours.

    JSON body: {"review_id": "...", "email": "..."}
    Returns:   {"staged": true, "expires_in_hours": 24, "review_id": "..."}
    """
    data      = request.get_json(silent=True) or {}
    review_id = data.get("review_id", "")
    email     = (data.get("email") or "").strip().lower()
    ...
    if _is_review_expired(result):
        _review_store.pop(review_id, None)
        return jsonify({"error": "Review session timed out."}), 410

    if email and _EMAIL_RE.match(email):
        result["pending_email"] = email
    ...
    if email and _EMAIL_RE.match(email):
        try:
            pdf_bytes = generate_report(result)
            meta = {
                "manuscript_title": result.get("manuscript_title", ""),
                "decision": result.get("decision", ""),
                "word_count": result.get("word_count", 0),
                "journal_name": result.get("journal_name", ""),
            }
            staged = stage_pending_payment(review_id, email, pdf_bytes, meta)
        except Exception:
            logger.warning("Could not stage PDF for dunning (non-fatal)")

    return jsonify({"staged": staged, "expires_in_hours": 24, "review_id": review_id})


def _deliver_staged_report(review_id: str, invoice_data: dict) -> None:
    """
    After successful payment, retrieve staged PDF from GCS (or regenerate) and
    email the full report + invoice to the user's verified address. Non-fatal.
    """
    if not EMAIL_ENABLED:
        return
    ...
    email = result.get("pending_email", "")
    if not email:
        pending = get_pending_payment(review_id)
        if pending:
    ...
    try:
        pending = get_pending_payment(review_id)
        pdf_bytes = (pending or {}).get("pdf_bytes") or generate_report(result)

        invoice_number = invoice_data.get("invoice_id", "N/A")
        payment_id     = invoice_data.get("payment_id", "")
        title          = result.get("manuscript_title", "Manuscript Review")
        decision       = result.get("decision", "See report")
        now            = datetime.datetime.utcnow()
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#333;padding:0">
  ...
  <div style="padding:28px;border:1px solid #e0e0e0;border-top:none;border-radius:0 0 8px 8px">
    <p>Dear Researcher,</p>
    <p>Thank you for your payment. Your <strong>full peer review report</strong> and <strong>invoice</strong> are attached.</p>
    <table style="width:100%;border-collapse:collapse;margin:20px 0;font-size:0.9rem">
      <tr style="background:#f7f7f7">
        <td style="padding:10px 14px;font-weight:bold;width:38%;border:1px solid #e8e8e8">Invoice No.</td>
        <td style="padding:10px 14px;border:1px solid #e8e8e8">{invoice_number}</td>
    ...
    <div style="background:#f0fffe;border:2px solid #00c9b1;border-radius:10px;padding:22px;margin-top:24px">
      <h3 style="color:#00875f;margin-top:0;font-size:1.05rem">Coming Soon: Document Revision Service</h3>
      <p style="margin-bottom:0">We are launching a <strong>personalised document revision service</strong> to help you strengthen your manuscript. <strong>Stay tuned!</strong></p>
    </div>
    <hr style="border:none;border-top:1px solid #ebebeb;margin:28px 0">
    <p style="color:#aaa;font-size:0.8rem;margin:0">Health Care Expert Reviews &mdash; reply to this email for any queries.</p>
  </div>
</body>
</html>"""
        ...
        _send_email(
            ...
            invoice_pdf,
            f"Invoice_{invoice_number}.pdf",
            attachments=[(pdf_bytes, f"PeerReview_Full_{safe_title}.pdf")],
        )
        logger.info(f"Dunning delivery: report + invoice sent to {email} for review {review_id}")
        delete_pending_payment(review_id)

    except Exception:
        logger.warning(f"_deliver_staged_report failed for review {review_id}", exc_info=True)
